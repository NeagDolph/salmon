...
export default {
  ...
  methods: {
    calcInputHeight(input) {
      ...
      this.$refs.commentInput.setAttribute("style", `height: ${this.$refs.commentInput.scrollHeight}px;`);
    },
    ...
    updateType(eventObj) {
      ...
      this.typetimer[currentUserId][this.commentSelectedIndex] = [
        setTimeout((passedInput) => {
          this.addComment(passedInput[0], passedInput[1], passedInput[2].typetimer[passedInput[3]][passedInput[1]][1])
        }, 500, [{userid: currentUserId, index: this.selectedIndex}, this.commentSelectedIndex, this, currentUserId]),
        inputValue
      ]
